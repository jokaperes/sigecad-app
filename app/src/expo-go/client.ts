...
export interface AcademicStartup {
  ...
  /** Raw current-period classes kept in memory only for deferred note hydration. */
  courses: Turma[];
...
export interface AcademicNotesHydration {
  overview: AcademicOverview;
  /** False means at least one note response failed and the snapshot must not be saved. */
  complete: boolean;
}

/** Coalesces same-tick academic calls into one concurrent WebView bridge batch. */
...
/** Initial foreground load: note calls share one same-origin bridge batch. */
...
/**
 * Fast Home path. The warm period response is consumed first, then identity,
 * classes, schedule and enrollment windows share one four-worker bridge batch.
 * Notes are intentionally deferred because Home does not need them.
 */
...
/**
 * Hydrates grade-only fields after Home is visible. A coalesced request lets
 * these calls share the same academic bridge batch as history and absences.
 * Partial responses remain useful in memory but are never marked complete,
 * preventing a failed note endpoint from looking like a deleted grade.
 */

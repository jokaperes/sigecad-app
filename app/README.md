...
nasce desativado para manter `window.open` e `target=_blank` na WebView do app.
...
A `MainApplication` e a `MainActivity` recusam saída HTTP/HTTPS como barreira
final. O redirect HTTP legado do SIGECAD é reescrito para HTTPS na mesma WebView,
e o documento do portal acadêmico fica oculto antes do primeiro desenho.
...
tests/                        11 core + 7 ciclo + 48 Expo Go + 12 segurança
```

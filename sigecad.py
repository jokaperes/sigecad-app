...
Uso:
    ...
    python3 sigecad.py            # poll + diff contra o ultimo estado
...
class _NoRedirect(urllib.request.HTTPRedirectHandler):
    def redirect_request(self, req, fp, code, msg, headers, newurl):
        # Redirect targets can contain CAS tickets. Never include them in errors.
...
class Client:
    def __init__(self, token):
        ...
        self.token = token
        # O instalador oficial do Python no macOS nem sempre aponta para um CA
        # bundle válido. Prefere certifi quando disponível, sem jamais desligar
        # a verificação TLS.
        try:
            import certifi
    ...
    def get_bytes(self, path, _retries=2):
        ...
        try:
            ...
            if e.code in (401, 403):
                raise AuthError(f"HTTP {e.code}")
            if e.code >= 500 and _retries > 0:  # transitório: tenta de novo
...
def current_period(client):
    ...
    # preferir o periodo cuja janela [data_inicio, data_fim] contem hoje;
    # fora de periodo (ferias), cai pro mais recente (maior id).
    ...
    for key, cur in new.items():
        ...
        if prev is None:
            if key.endswith("::resultado") or key.endswith("::faltas"):
                continue  # baseline dessas nao vira evento
